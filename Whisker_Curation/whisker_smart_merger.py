...
def calculate_straightness(x_coords, y_coords):
    """
    Calculate how straight a whisker is.
    Returns ratio of direct distance to path length (0-1, 1 = perfectly straight).
    """
    if len(x_coords) < 2:
        return 0.0
    
    direct_dist = euclidean((x_coords[0], y_coords[0]), (x_coords[-1], y_coords[-1]))
    path_length = calculate_whisker_length(x_coords, y_coords)
...
def score_merge_candidate(w1, w2, distance_threshold=30.0, fixture_points=None):
    ...
    Criteria:
    - Fixture point protection (NEVER merge bases near fixture points)
    - Collinearity (are they pointing same direction?)
    - Endpoint proximity (are endpoints close?)
    - Overlap/similarity (do they occupy similar space?)
    - Length similarity (similar lengths suggest split whisker)
    - Straightness (noise tends to be less straight)
    ...
    # CRITICAL: Check fixture points - NEVER merge whisker bases at fixture points
    ...
    # 1. Check collinearity (VERY important - split whiskers should be aligned)
    is_collinear = check_collinearity(w1, w2, angle_tolerance=np.pi/4)  # More lenient: 45 degrees
    if is_collinear:
        score += 50
        reasons.append('collinear')
    else:
        # If not collinear, heavily penalize (likely different whiskers)
        score -= 80
        reasons.append('not_collinear')
    
    # 2. Check endpoint distances
    ...
    # Reward close endpoints (split whiskers should have close endpoints)
    ...
    # 3. CRITICAL: Penalize connections involving fixture bases
    ...
    # 4. Penalize base-to-base or tip-to-tip connections (creates U-shapes)
    ...
    # 5. Check for overlap/similarity (split whiskers often occupy similar space)
    overlap = calculate_overlap_score(w1, w2)
    if overlap > 30:
        score += 30
        reasons.append(f'high_overlap({overlap:.0f})')
    elif overlap > 15:
        score += 15
        reasons.append(f'moderate_overlap({overlap:.0f})')
    
    # 6. Length similarity (split whiskers often have similar lengths)
    if w1['length'] > 0 and w2['length'] > 0:
        length_ratio = min(w1['length'], w2['length']) / max(w1['length'], w2['length'])
        if length_ratio > 0.5:
            score += 25
            reasons.append('similar_length')
        elif length_ratio < 0.3:
            score -= 10
            reasons.append('very_different_length')
    
    # 7. Straightness (straight whiskers are more likely real)
    straight1 = calculate_straightness(w1['x_coords'], w1['y_coords'])
    straight2 = calculate_straightness(w2['x_coords'], w2['y_coords'])
    avg_straightness = (straight1 + straight2) / 2
    
    if avg_straightness > 0.8:
        score += 10
        reasons.append('very_straight')
    elif avg_straightness < 0.5:
        score -= 15
